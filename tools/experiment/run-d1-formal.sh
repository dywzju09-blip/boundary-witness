...
usage() {
  cat >&2 <<'EOF'
usage: run-d1-formal.sh --repo-root <source-dir> --runs-root <runs-dir> --commit <git-commit> --deployment-sha256 <sha256> [options]
...
Options:
  ...
  --campaigns <n>               Number of update_hook formal campaigns. Default: 30
  --cpu-minutes <n>             Per-campaign max_total_time in minutes. Default: 30
  --safe-cpu-minutes <n>        Safe-only control max_total_time in minutes. Default: same as --cpu-minutes
EOF
}

fail() {
  printf 'run-d1-formal: %s\n' "$*" >&2
  exit 1
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --cpu-minutes)
      ...
      ;;
    --safe-cpu-minutes)
      [[ $# -ge 2 ]] || fail "--safe-cpu-minutes requires a value"
      safe_cpu_minutes="$2"
      shift 2
      ;;
    -h|--help)
...
if [[ -z "$safe_cpu_minutes" ]]; then
  safe_cpu_minutes="$cpu_minutes"
fi
[[ "$safe_cpu_minutes" =~ ^[0-9]+$ && "$safe_cpu_minutes" -gt 0 ]] || fail "--safe-cpu-minutes must be a positive integer"
...
objective_digest="$(sha256_file "$objective_path")"
run_id="unix$(date +%s)-${commit:0:7}-d1formal"
...
manifest = {
    ...
    "objective_config_digest": os.environ["OBJECTIVE_DIGEST"],
...
for index in $(seq 1 "$campaigns"); do
  campaign_id="$(printf 'd1-uh-formal-%03d' "$index")"
  seed=$((3000 + index))
...
PY
  ...
  (
    cd "$shared_dir"
    BW_D1_COUNTERS_PATH="$counters_path" cargo +"$rustup_toolchain" fuzz run update_hook_actions "${partial_run}/input/update-hook-corpus" -- \
...
PY
    ...
    FINDING_CAMPAIGN_ID="$campaign_id" \
    FINDING_DIGEST="$digest" \
    FINDING_RULE="$primary_rule_id" \
    FINDING_SIGNATURE="$normalized_signature" \
    python3 - "${partial_run}/findings.jsonl" <<'PY'
import json
import os
import sys
record = {
    "campaign_id": os.environ["FINDING_CAMPAIGN_ID"],
    "artifact_digest": os.environ["FINDING_DIGEST"],
    "primary_rule_id": os.environ["FINDING_RULE"],
    "normalized_signature": os.environ["FINDING_SIGNATURE"],
}
...
record = {
    "schema_version": "boundary-witness.d1-campaign-record/0.1",
...
safe_dir="${partial_run}/artifacts/d1-uh-safe-formal"
mkdir -p "${safe_dir}/artifacts" "${safe_dir}/logs"
safe_counters="${safe_dir}/counters.json"
safe_seconds=$((safe_cpu_minutes * 60))
set +e
(
  cd "$shared_dir"
  BW_D1_COUNTERS_PATH="$safe_counters" cargo +"$rustup_toolchain" fuzz run update_hook_safe_only "${partial_run}/input/update-hook-corpus" -- \
    -max_total_time="$safe_seconds" \
    -seed=9901 \
...
if [[ ! -s "$safe_counters" ]]; then
  python3 - "$safe_counters" <<'PY'
import json
import sys
with open(sys.argv[1], "w", encoding="utf-8") as f:
    json.dump({
...
SAFE_CPU_MINUTES="$safe_cpu_minutes" \
...
with open(os.environ["SAFE_COUNTERS"], encoding="utf-8") as f:
    safe_counters = json.load(f)

total_executions = sum(record["executions"] for record in records)
valid = sum(record["valid_sequence_count"] for record in records)
invalid = sum(record["invalid_sequence_count"] for record in records)
summary = {
    "schema_version": "boundary-witness.d1-formal-summary/0.1",
    "mode": "formal",
    "campaigns": records,
    "campaign_count": len(records),
    "required_primary_success_campaigns": 18,
    "primary_found_campaigns": sum(1 for record in records if record["outcome"] == "primary_found"),
    "timeout_campaigns": sum(1 for record in records if record["outcome"] == "timeout"),
    "tool_error_campaigns": sum(1 for record in records if record["outcome"] == "tool_error"),
    "progress_campaigns": sum(1 for record in records if record["progress_count"] > 0),
    "secondary_campaigns": sum(1 for record in records if record["secondary_count"] > 0),
    "executions": total_executions,
    "valid_sequence_count": valid,
    "invalid_sequence_count": invalid,
    "valid_sequence_ratio_ppm": 0 if total_executions == 0 else valid * 1_000_000 // total_executions,
    ...
    "time_to_first_primary_ms": [
        record["time_to_first_primary_ms"]
        for record in records
        if record["time_to_first_primary_ms"] is not None
    ],
    "safe_only": {
        "target": "update_hook_safe_only",
        "seed": 9901,
        "cpu_minutes": int(os.environ["SAFE_CPU_MINUTES"]),
        "exit_status": int(os.environ["SAFE_STATUS"]),
        "artifact_count": int(os.environ["SAFE_ARTIFACT_COUNT"]),
        "counters": safe_counters,
    },
}
summary["success_threshold_met"] = summary["primary_found_campaigns"] >= summary["required_primary_success_campaigns"]
