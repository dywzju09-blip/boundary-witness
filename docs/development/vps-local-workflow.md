...
远端执行器（未跟踪，披露判断未决）在链路走中继时会因超时而失败，且失败信息看起来像「远端不可用」，容易误判成主机宕机。**已实测的实际情况：**

| 观察 | 值 |
| --- | --- |
| ICMP 往返 | 约 288 ms，丢包 0 |
| 单次 `ssh ... test -f <marker>` 建连耗时 | 成功时约 13.5 s，常见超时 |
| 原设置 `ConnectTimeout` | 10 s——不够 |
| 原设置 keepalive 容忍（`ServerAliveInterval` × `CountMax`） | 15 s × 3 = 45 s，rsync 传大树时中途被判死 |

2026-07-31 调整为：`ConnectTimeout=30`、安全标记检查外层 `timeout 60s`、keepalive 容忍 `15 s × 20 = 300 s`。调整后一次通过。

**判断方法**：先 `ping` 再 `ssh`。**ICMP 通而 SSH 22 端口超时，是链路慢不是主机宕**；这种情况调超时即可，不要按「远端不可用」处理，更不得退回本机执行重活。

若链路长期走中继，上述值应保留。相关约束见 [AGENTS](../../AGENTS.md)。
