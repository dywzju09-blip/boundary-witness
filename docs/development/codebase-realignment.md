...
2026-08-04 再次调整：**关系正确性排到最前，随后先完成单目标核心闭环，猎物探针用于决定是否扩大。**
...
| 6 | `is_unsafe_fn` + safe-entry lineage + `RustContractFact` | 3–5 | 中 | ⬜ |
| 7 | 真实外部 IR 获取 + artifact binding | 6 | 中 | ⬜ |
| 8 | 外部侧新 crate：Q1 → Q4′ → 降级 Q3 | 7 | 大 | ⬜ |
| 9 | `SiteDescriptor` 扩展 + 分层身份 + 外部符号记录 | 6–8 的记录形状 | 中 | ⬜ |
| 10 | 一次性 schema 升版（D2） | 8、9 定稿 | 中 | ⬜ |
| 11 | 联合关系判定器 | 10 | 大 | ⬜ |
| 12 | 反证生成器重写（D3）+ 独立 oracle 闭环 | 11 | 大 | ⬜ |
| 13 | Tier A 判据修正 + PP 批量驱动器 | 5–7 | 小 | ⬜ |
| 14 | 小样本 Gate P / Gate C0 | 12、13 | 实验 | ⬜ |

**两条硬约束：**

- **第 2 步完成前不要做第 7 步及以后。** 关系错了，后面所有实现都在实现错的判据。
- **第 12 步完成前不进入 50–100+ crate 评估。** Gate P No-Go 时保留第 1–12 步的核心闭环，停止扩大并转路线 B/C/D。

第 1–13 步不以 Gate P 通过为启动条件。

## 10. 本次审计明确不做的事

- **不做全局重构。** 最大最贵的资产（编译器 Rust 侧）在新路线中价值上升；身份模型是 builder，加字段不动既有调用点；外部侧是 0 行，属纯新增；30k 行测试资产会被重构大批作废，而这些测试正是「非空性检查」纪律的载体。
- **不做「先清理再干活」。** 除 §7.1 一项外，不在核心闭环之前做无关清理。
