...
1. 照 [`unconstrained_return_lifetime_relation`](../../compiler/bw-rustc/src/rustc_api/mir.rs) 的形式，新增一个从 HIR 签名判断「回调参数的生命周期 bound 是否绑在 `&'c self` 而非 `'static`」的检查；
2. 输出是**候选 API 列表**，不是结论——候选仍需经阶段 A 的证明链才能升级；
