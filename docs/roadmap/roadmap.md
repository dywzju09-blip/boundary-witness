...
路线的终点不是「一个能验证 rusqlite 的工具」。按
[scope §3.1](../project/scope-and-boundaries.md) 的发现层级：

```text
T1 切片验证          给定 API 清单，验证该切片上有没有缺陷
   ↓                 ← 单目标 Core Complete（阶段 6）到这里
T2 未披露实例发现     N 个 crate 跑遍版本，找出此前无人报过的实例
   ↓                 ← 阶段 9 的「冻结后独立确认发现」到这里，**这是本工作的终点**
T3 无先验发现         任意 crate，自动识别哪些 API 是回调注册
                      ← **不在本路线内**，future work
```

**T2 产出的是真实的 0-day**——没有公开记录的缺陷，可走披露流程。它与 T3 的差别不在结论
性质，而在候选池由谁圈定：T2 的池子由人写的 API 清单圈定。

**每个阶段都服务 T1 → T2 这条线。** 判断一项工作要不要排进路线，先问它是否让「在已有
清单的 crate 上找出未披露实例」这件事更接近成立；只让 T3 更接近的工作一律不排。
