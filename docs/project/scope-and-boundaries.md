...
但外部持有期那一半的证据是 `register_call` / `unregister_call` 等事实，而这些事实**本身就是 API map 分类出来的**——`registration` 那一步靠 map 才知道哪个 FFI 调用是注册、哪个是注销。所以：

- **接入一个新组件仍然必须先有人手写 API map。** 省掉的是版本边界，不是 map 本身。
- API map registry 目前也不是「所有组件无需改 compiler 即可扩展」的统一语义注册层。
