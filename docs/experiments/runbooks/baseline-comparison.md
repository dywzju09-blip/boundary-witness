...
本 runbook 描述如何在同一 corpus 上构建并运行 Yuga 与 FFIChecker。

> **这个实验已经跑过，且结论是反例。** 2026-07-31 的运行证明 Yuga 能报出主线缺陷类的 5/7（见 [结果](../results/gate0-baseline-comparison-2026-07-31.md)、[误报归因](../results/gate0-yuga-precision-triage-2026-07-31.md)），当时的立论已随之作废。
>
> **本文现在的用途是工具构建的操作参考**——[规模化精度对照](precision-comparison-at-scale.md) 与后续所有 baseline 实验都引用本文的环境障碍与绕过。它不再是一个待决的判定实验。当前研究 gate 见 [milestone gates](../../roadmap/milestone-gates.md)。

## 这件事当初决定什么

原假设是「现有工作不建模外部持有者，所以不会报这一缺陷类」。**Yuga 是最近的反例候选**——它检测函数签名上的生命周期标注错误，问题陈述与本系统的持有期维度表面相近。

在几周的外部侧分析投入之前先跑这一步，是为了让反例尽早暴露而不是事后暴露。**这个判断是对的：反例确实出现了，且出现在投入之前。** 同样的理由现在适用于 [Gate P 猎物存在性探针](prey-existence-probe.md)。
...
| 观察 | 结论 | 动作 |
| --- | --- | --- |
| 正对照非空，目标样本 0 报 | 该工具确实不覆盖此缺陷类 | 记录报告，写入 related work 的区分依据 |
| 正对照非空，目标样本报中若干 | **需逐条归因** | 比对它报的是不是同一回事（同一函数？同一理由？）。若确为同一缺陷，相关立论重做 |
| 正对照为空 | 结果无效 | 修环境，不得出任何结论 |

**注意区分"报了同一个函数"与"报了同一个缺陷"。** 若某工具因为别的理由（例如某个无关的返回引用）碰巧命中同一函数，那不构成同类命中，但必须在记录中说清。**2026-07-31 的运行落在第二行**：5 条命中且理由确为同一缺陷。
...
在 [Gate P、A、B、D](../../roadmap/milestone-gates.md) 全部通过前，不得在任何对外材料中表述跨语言契约不相容判定已达成。
