...
use rusqlite_lab_shared::{update_hook::UpdateHookConnection, OwnedCounter};

#[test]
fn safe_owned_update_hook_events_have_generic_order_and_no_core_finding() {
    let sink = Arc::new(MemorySink::default());
    let runtime = RuntimeContext::new("run:update".into(), "trace:update".into(), sink.clone());
    runtime
        .emit_trace_start(BuildId::from("build:test"))
        .unwrap();

    let connection = UpdateHookConnection::open(runtime.clone(), site("site:connection")).unwrap();
    let counter = Tracked::new(runtime.clone(), site("site:object"), OwnedCounter::new());
    let token = connection.register(site("site:callback")).unwrap();
    token
        .bind_object(counter.id(), &site("site:object"))
        .unwrap();
    token.invoke(site("site:invoke")).unwrap();
    connection
        .unregister(&token, site("site:unregister"))
        .unwrap();
    drop(counter);
...
#[test]
fn borrowed_update_hook_sequence_uses_static_contract_and_runtime_evidence() {
    let sink = Arc::new(MemorySink::default());
    let runtime = RuntimeContext::new("run:update".into(), "trace:update".into(), sink.clone());
    runtime
        .emit_trace_start(BuildId::from("build:test"))
    ...
    let finding = summary
        .finding("BW-LIFE-002")
        .expect("borrowed retained callback invoked after drop should be detected");
    assert!(finding
        .evidence
        .iter()
...
#[test]
fn replacing_update_hook_releases_previous_callback() {
    let sink = Arc::new(MemorySink::default());
    let runtime = RuntimeContext::new("run:update".into(), "trace:update".into(), sink.clone());
    runtime
        .emit_trace_start(BuildId::from("build:test"))
    ...
    let connection = UpdateHookConnection::open(runtime.clone(), site("site:connection")).unwrap();
    let first = connection.register(site("site:callback:first")).unwrap();
    let second = connection.register(site("site:callback:second")).unwrap();
...
#[test]
fn closing_update_hook_owner_releases_retained_callback_before_owner_drop() {
    let sink = Arc::new(MemorySink::default());
    let runtime = RuntimeContext::new("run:update".into(), "trace:update".into(), sink.clone());
    runtime
        .emit_trace_start(BuildId::from("build:test"))
...
#[test]
fn borrowed_no_trigger_update_hook_stays_exposure_only() {
    let sink = Arc::new(MemorySink::default());
    let runtime = RuntimeContext::new("run:update".into(), "trace:update".into(), sink.clone());
    runtime
        .emit_trace_start(BuildId::from("build:test"))
...
#[test]
fn unregister_before_drop_update_hook_has_no_core_or_exposure_finding() {
    let sink = Arc::new(MemorySink::default());
    let runtime = RuntimeContext::new("run:update".into(), "trace:update".into(), sink.clone());
    runtime
        .emit_trace_start(BuildId::from("build:test"))
...
fn contract() -> CallbackRetentionContract {
    CallbackRetentionContract {
        ...
        contract_id: "contract:update-hook".to_owned(),
        ...
        api_entries: vec![
            CallbackApiEntry {
                clause_id: "clause:register-retains".to_owned(),
                api_id: "api:rusqlite:update_hook".to_owned(),
