...
fn driver_writes_clean_observation_for_empty_findings() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_bw-v3-nday-adapter"))
        .env("BW_V3_NDAY_CASE_ROOT", &fixture.case_root)
        .env("BW_BLIND_CASE_ID", "blind-0123456789abcdef")
        .env("BW_BLIND_SUITE_ID", "suite.v3-1.nday.gate.001")
...
fn static_facts() -> &'static str {
    r#"{"schema_version":"bw.static/0.1","record_id":"fact:object","producer":"test","build_id":"build:test","payload":{"kind":"object_site","site_id":"site:buffer:server","semantic_site_key":"semantic:buffer:server","type_name":"TrackedBuffer"}}
"#
}
